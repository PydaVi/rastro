"""
Bloco 6d integration test — CreateAccessKey Pivot.

Fluxo testado:
  mesh-dispatch-operator (iam:CreateAccessKey on cache-sync-bot)
    → extracted://iam_user/cache-sync-bot
      → assume runtime-broker-role

Usa o fixture pre-construído (create_access_key_pivot_real).
"""
...
from operations.service import run_generated_campaign, _derive_create_access_key_hypotheses
from app.main import execute_run

LAB_DIR = Path(__file__).parent.parent / "terraform-realistic-iam" / "create_access_key_pivot_real" / "rastro_local"
FIXTURE_PATH = LAB_DIR / "aws_create_access_key_pivot_lab.local.json"
OBJECTIVE_PATH = LAB_DIR / "objective_aws_create_access_key_pivot.local.json"
SCOPE_PATH = LAB_DIR / "scope_aws_create_access_key_pivot_openai.local.json"

OUTPUT = Path("outputs_bloco6d_create_access_key")
ACCOUNT = "550192603632"
REGION = "us-east-1"
ENTRY_USER_ARN = f"arn:aws:iam::{ACCOUNT}:user/mesh-dispatch-operator"
TARGET_ROLE_ARN = f"arn:aws:iam::{ACCOUNT}:role/runtime-broker-role"


def main() -> None:
    discovery_snapshot = json.loads(FIXTURE_PATH.read_text())

    hypotheses = _derive_create_access_key_hypotheses(discovery_snapshot, [ENTRY_USER_ARN])
    print(f"\n{'='*60}")
    ...
    if not hypotheses:
        print("ERRO: nenhuma hipótese gerada — verifica createkey_by no fixture")
        sys.exit(1)

    pivot_hyp = next((h for h in hypotheses if h.attack_class == "iam_create_access_key_pivot"), None)
    if not pivot_hyp:
        print("ERRO: nenhuma hipótese iam_create_access_key_pivot")
        sys.exit(1)

    campaign_output = OUTPUT / "campaigns" / "aws-iam-create-access-key-pivot" / "bloco6d-createkey"
    ...
    plan = {
        "id": "bloco6d-createkey-test",
        "profile": "aws-iam-create-access-key-pivot",
    ...
    target = TargetConfig(
        name="rastro-create-access-key-pivot-lab",
        accounts=[ACCOUNT],
        allowed_regions=[REGION],
        entry_roles=[],
        entry_credential_profiles={ENTRY_USER_ARN: "rastro-createkey-entry"},
    )
    ...
    authorization = AuthorizationConfig(
        ...
        permitted_profiles=["aws-iam-create-access-key-pivot"],
        permitted_entry_identities=[ENTRY_USER_ARN],
        planner_config={"backend": "openai", "model": "gpt-4o"},
    )

    print(f"\n{'='*60}")
    print("Iniciando run real — aws-iam-create-access-key-pivot")
    ...
    if result.report_json and Path(result.report_json).exists():
        ...
        print(f"flags:         {flags}")
        rollback = report.get("rollback_errors", [])
        print(f"rollback_ok:   {rollback == []}")
