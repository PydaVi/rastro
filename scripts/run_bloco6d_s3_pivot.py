"""
Bloco 6d integration test — S3 Object Pivot.

Fluxo testado:
  asset-manifest-user (s3:GetObject mesh-artifacts-550192603632/exports/runtime/bootstrap.json)
    → lê objeto com credenciais de delivery-bot-user
      → assume delivery-broker-role como identidade extraída
"""
...
LAB_DIR = Path(__file__).parent.parent / "terraform-realistic-iam" / "s3_object_pivot_real" / "rastro_local"
FIXTURE_PATH = LAB_DIR / "aws_s3_object_pivot_lab.local.json"
OBJECTIVE_PATH = LAB_DIR / "objective_aws_s3_object_pivot.local.json"
SCOPE_PATH = LAB_DIR / "scope_aws_s3_object_pivot_openai.local.json"

OUTPUT = Path("outputs_bloco6d_s3_pivot")
ACCOUNT = "550192603632"
REGION = "us-east-1"
ENTRY_USER_ARN = f"arn:aws:iam::{ACCOUNT}:user/asset-manifest-user"
TARGET_ROLE_ARN = f"arn:aws:iam::{ACCOUNT}:role/delivery-broker-role"
...
def main() -> None:
    ...
    pivot_hyp = next((h for h in hypotheses if h.attack_class == "s3_pivot" and h.target == TARGET_ROLE_ARN), None)
    if not pivot_hyp:
        print("ERRO: nenhuma hipótese s3_pivot para target_role")
        sys.exit(1)

    campaign_output = OUTPUT / "campaigns" / "aws-credential-pivot-s3" / "bloco6d-s3"
    ...
    plan = {
        "id": "bloco6d-s3-test",
        "profile": "aws-credential-pivot-s3",
    ...
    target = TargetConfig(
        name="rastro-s3-pivot-lab",
        accounts=[ACCOUNT],
        allowed_regions=[REGION],
        entry_roles=[],
        entry_credential_profiles={ENTRY_USER_ARN: "rastro-s3-pivot-entry"},
    )
    ...
    authorization = AuthorizationConfig(
        ...
        permitted_profiles=["aws-credential-pivot-s3"],
        permitted_entry_identities=[ENTRY_USER_ARN],
        planner_config={"backend": "openai", "model": "gpt-4o"},
    )

    print(f"\n{'='*60}")
    print("Iniciando run real — aws-credential-pivot-s3")
