...
COMPUTE_PIVOT_APP_SYNTHETIC_PROFILES: dict[str, ProfileDefinition] = {
    "aws-iam-s3": ProfileDefinition(
        ...
        fixture_path=_path("fixtures", "compute_pivot_app_unified_lab.json"),
...
MIXED_GENERALIZATION_SYNTHETIC_PROFILES: dict[str, ProfileDefinition] = {
    ...
    "aws-cross-account-data": ProfileDefinition(
        name="aws-cross-account-data",
        bundle="aws-enterprise-mixed-synthetic",
        description="Synthetic cross-account path for mixed-generalization benchmark.",
        fixture_path=_path("fixtures", "mixed_generalization_cross_account_lab.json"),
        objective_path=_path("examples", "objective_compute_pivot_app_cross_account.json"),
        scope_path=_path("examples", "scope_mixed_generalization_cross_account.json"),
    ),
    "aws-multi-step-data": ProfileDefinition(
        name="aws-multi-step-data",
        bundle="aws-enterprise-mixed-synthetic",
        description="Synthetic multi-step path for mixed-generalization benchmark.",
        fixture_path=_path("fixtures", "mixed_generalization_multi_step_lab.json"),
        objective_path=_path("examples", "objective_compute_pivot_app_multi_step.json"),
...
PROFILE_SETS: dict[str, dict[str, ProfileDefinition]] = {
    "internal-data-platform": INTERNAL_DATA_PLATFORM_SYNTHETIC_PROFILES,
    "serverless-business-app": SERVERLESS_BUSINESS_APP_SYNTHETIC_PROFILES,
    "compute-pivot-app": COMPUTE_PIVOT_APP_SYNTHETIC_PROFILES,
    "mixed-generalization": MIXED_GENERALIZATION_SYNTHETIC_PROFILES,
}


def get_synthetic_profile(profile_set: str, family: str) -> ProfileDefinition:
    try:
        return PROFILE_SETS[profile_set][family]
    except KeyError as exc:
        raise KeyError(f"unknown synthetic profile set: {profile_set}") from exc


def get_mixed_synthetic_profile(family: str, candidate_or_plan: dict | None = None) -> ProfileDefinition:
    fixture_set = "mixed-generalization"
    if candidate_or_plan:
        fixture_set = candidate_or_plan.get("execution_fixture_set") or fixture_set
    return get_synthetic_profile(fixture_set, family)
