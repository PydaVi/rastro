...
## Step-by-Step
```
1. ActionType.ENUMERATE analyst -> account (tool=iam_list_roles) | success=True | backend=mock | reason=Selected highest-priority action enumerate.
...
3. ActionType.ACCESS_RESOURCE AuditRole -> sensitive_bucket (tool=s3_read_sensitive) | success=True | backend=mock | reason=Selected highest-priority action access_resource.
```
